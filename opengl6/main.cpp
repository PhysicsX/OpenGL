...
template<int i>
void RenderSquareTextured(float offset,auto texture)
{
     glPushMatrix();
     ...
     if(first)  //build a list
     {
         ...
         glBegin(GL_QUADS);
         glColor3f( 1.0f, i, 0.0f );glTexCoord2f(0.0f, 0.0f);
         glVertex3f(offset-1.0f/2, -1.0f/2, 1.0f/2+offset);
         glColor3f( 0.0f, i, 0.0f );glTexCoord2f(1.0f, 0.0f);
         glVertex3f(offset+1.0f/2, -1.0f/2, 1.0f/2+offset);
         glColor3f( 1.0f, i, i );glTexCoord2f(1.0f, 1.0f);
         glVertex3f(offset+1.0f/2, 1.0f/2, 1.0f/2+offset);
         glColor3f( 1.0f, i, 0.0f );glTexCoord2f(0.0f, 1.0f);
         glVertex3f(offset-1.0f/2, 1.0f/2, 0.9f/2+offset);
         glEnd();
...
void startLightDance()
{
    static float angle=0;
    angle=angle-1;
    {float a[4]={sin((angle+4.5)*3.1415926/180),cos((angle+4.5)*3.1415926/180),sin(2*(angle+4.5)*3.1415926/180), 0};glLightfv(GL_LIGHT0 ,GL_DIFFUSE ,a);}
    {float a=.4;glLightfv(GL_LIGHT0,GL_SPOT_EXPONENT,&a);}
    {float a[4]={0,0,0,1};glLightfv(GL_LIGHT0,GL_POSITION ,a);}

    {
        float a[4]={0,0,0,0};
        a[0]=1*sin(angle*3.1415926/180);a[1]=1*cos(angle*3.1415926/180);a[2]=0;
...
void someThingRandom(float offset=0)
{
    glPushMatrix();
    static __thread float f=0;f=f+.1;
    glRotated( f, 1.0f, 1.0f, 1.0f );   //u may enter the vector of the axis along which the plane may be rotated
    glBegin(GL_LINES );

    double a=0,s=0,i=-1,j=-1,k=0;
    while(a<360)
    {
        while(s<360)
        {
            i=.8*sin(a*3.1415926/180)*cos(s*3.1415926/180);
            j=.8*sin(a*3.1415926/180)*sin(s*3.1415926/180);
            k=.8*cos(a*3.1415926/180);
            //k=.04/k;
            glColor3f(1-j*2,i*10,k*20);glVertex3f(i*.7+offset,j*.7,k*.7);
